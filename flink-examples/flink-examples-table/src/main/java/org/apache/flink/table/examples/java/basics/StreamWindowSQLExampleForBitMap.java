 ...
 */
public class StreamWindowSQLExampleForBitMap {
    private static final Logger logger = LoggerFactory.getLogger(StreamWindowSQLExampleForBitMap.class);
...
//                .build();
        ...
        String dedupSql = "create view dedup as select * from (select *, row_number() over (partition by ticket_no order by proc_time desc) as rn from ticket) tmp where tmp.rn = 1";

        tEnv.executeSql(dedupSql);

        String aggSql = "create view agg as select substring(create_time,1,10) as create_date,group_id,bitMapCountWithRetract(ticket_no) filter (where status <> 50) as cnt from dedup group by substring(create_time,1,10),group_id";
